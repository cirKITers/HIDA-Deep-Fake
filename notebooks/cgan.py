...
batch_size = 512
epochs = 100
sample_size = 64  # fixed sample size for output images
...
noise_source = NoiseSource(
    seed=seed, noise_gain=noise_gain, noise_offset=noise_offset
).to(device)
...
for epoch in range(epochs):
    ...
    for bi, data in tqdm(
        ...
        for step in range(k):
            data_fake = generator(noise_source((b_size, nz))).detach()
            data_real = image
            # train the discriminator network
            loss_d += train_discriminator(optim_d, data_real, data_fake)

        data_fake = generator(noise_source((b_size, nz)))
